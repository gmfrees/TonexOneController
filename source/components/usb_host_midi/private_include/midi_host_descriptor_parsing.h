...
typedef const usb_standard_desc_t *midi_func_array_t[]; // Array of pointers to const usb_standard_desc_t
...
/**
 * @brief Parse MIDI interface descriptor
 *
 * #. Check if the required interface exists
 * #. Parse the interface descriptor
 * #. Check if the device is MIDI compliant
 * #. For MIDI compliant devices also parse second interface descriptor and functional descriptors
 *
 ...
 * @param[out] info_ret   Array of parsed information, see midi_parsed_info_t
 * @return
 *     - ESP_OK:            Success
...
/**
 * @brief Print MIDI specific descriptor in human readable form
 *
 ...
 *
 * @param[in] _desc MIDI specific descriptor
 */
